...
# Grace period constant (must match socket_manager)
GRACE_PERIOD = 30000
...
CSV_FILE = os.path.join(
    os.path.dirname(__file__), "..", "..", "planomics-policy-cards.csv"
)
...
class TeamDisconnectPayload(BaseModel):
    team_id: str


@router.post("/team-disconnect")
async def team_disconnect_beacon(payload: TeamDisconnectPayload):
    """
    Endpoint for navigator.sendBeacon when tab closes.
    Starts grace period removal for the team (not immediate).
    """
    team_id = str(payload.team_id)
    logger.info(f"[Beacon] Received disconnect beacon for team {team_id}")

    # Import here to avoid circular imports
    from socket_manager import (
        connected_teams,
        disconnect_timers,
        broadcast_online_teams,
        socket_to_team,
    )
    import asyncio

    if team_id in connected_teams:
        # Set status to reconnecting (grace period)
        ...
        async def remove_after_grace():
            ...
            if (
                ...
                logger.info(f"[Beacon] Grace period expired, removing team {team_id}")
        ...
        for sid, tid in list(socket_to_team.items()):
            if tid == team_id:
        ...
        # Broadcast update
        await broadcast_online_teams()
        logger.info(f"[Beacon] Team {team_id} grace period started")
