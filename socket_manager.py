...
from models import AuctionState, AuctionStatus, Bid, Plot, PlotStatus, Team
...
sio = socketio.AsyncServer(
    async_mode="asgi", cors_allowed_origins="*", ping_timeout=25, ping_interval=10
)
...
def get_connected_teams() -> list[str]:
    ...
    names = [
        info["team_name"] for info in _team_presence.values() if info.get("team_name")
    ]
    names += [
        info.get("team_name", info["role"].capitalize())
        for info in _other_clients.values()
    ]
    return names
...
async def broadcast_connection_count():
    ...
    try:
        await sio.emit(
            "connection_count",
            {"count": get_connected_count(), "teams": get_connected_teams()},
            room="auction_room",
        )
...
async def disconnect(sid):
    ...
    if team_id:
        ...
        if current_info and current_info.get("sid") == sid:
            ...
            logger.info(
                f"Team {team_id} ({current_info.get('team_name')}) is now offline."
            )
            await broadcast_connection_count()
        else:
            # A newer SID already replaced this one (page refresh). Do nothing.
            logger.info(
                f"Stale disconnect for team {team_id} (sid {sid}). Current sid is {current_info.get('sid') if current_info else 'none'}. Ignoring."
            )
    else:
    ...
    try:
        ...
        if team_id:
            ...
            if existing and existing["sid"] != sid:
                ...
                logger.info(
                    f"Team {team_id} reconnecting. Kicking old sid {old_sid}, accepting new sid {sid}."
                )
            ...
            try:
                ...
                async with async_session2() as s2:
                    ...
                    if team and getattr(team, "is_banned", False):
                        logger.warning(
                            f"Banned team {team_id} attempted to join auction."
                        )
                        await sio.emit(
                            "banned",
                            {"message": "Your team has been banned from the auction."},
                            room=sid,
                        )
            ...
            logger.info(
                f"Team {team_name} (id={str_team_id}) registered with sid {sid}."
            )
            ...
            async with async_session() as session:
                ...
                await sio.emit(
                    ...
                    serialize(
                        {
                            "status": state.status,
                            "current_plot": current_plot.dict()
                            if current_plot
                            else None,
        ...
        logger.error(
            f"CRITICAL: Unhandled error in join_auction for {sid}: {e}", exc_info=True
        )
...
async def place_bid(sid, data):
    ...
    async with async_session() as session:
        ...
        if getattr(team, "is_banned", False):
            await sio.emit(
                "bid_error",
                {"message": "Your team has been banned and cannot place bids."},
                room=sid,
            )
            return
        ...
        if state.round4_phase == "bid":
            ...
            seller_offer_stmt = (
                select(RebidOffer)
                .where(
                    RebidOffer.plot_number == plot.number,
            ...
            if seller_offer and seller_offer.offering_team_id == team.id:
                await sio.emit(
                    "bid_error",
                    {"message": "You cannot bid on a plot you are selling!"},
                    room=sid,
                )
                return
        ...
        min_required = (
            current_highest + 100000
            if current_highest > 0
            else (adjusted_base or 100000)
        )

        if amount < min_required:
            await sio.emit(
                "bid_error",
                {"message": f"Minimum bid is ₹{min_required:,.0f}"},
                room=sid,
            )
            return
        ...
        if plot.winner_team_id is None:
            plot.purchase_price = Decimal(amount)
